...
public class TaskController {
	...
	public String index(Model model) {
		...
		List<Task> openTask =  tasks.stream()
				.filter(t -> t.getStatus().equals("Open"))
				.collect(Collectors.toList());

		List<Task> inProgressTask =  tasks.stream()
				.filter(t -> t.getStatus().equals("In Progress"))
				.collect(Collectors.toList());

		List<Task> finishedTask =  tasks.stream()
				.filter(t -> t.getStatus().equals("Finished"))
				.collect(Collectors.toList());

		model.addAttribute("openTasks",openTask);
		model.addAttribute("inProgressTasks",inProgressTask);
		model.addAttribute("finishedTasks",finishedTask);
